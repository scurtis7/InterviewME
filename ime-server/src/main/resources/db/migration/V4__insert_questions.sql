...
and you increment it ( i++ ) in one instance, the change will be reflected in all instances.'),
    ...
    - Bonus Question: Why don’t constructors have a return type?
       ...
       ('JAVA', 'MEDIUM',
        'What is a local variable, an instance variable and a class variable? Contrast some of the differences between them? When would you use an instance variable as opposed to a local variable? When would you use a class variable?', '- Local variables are defined inside methods, constructors or blocks. They will be declared and initialized within the method and destroyed when the method has completed.
...
- sorted
'),
       ('JAVA', 'MEDIUM', 'Explain the Comparable interface.',
        'You implement the Comparable interface if you want to sort a pojo.  You would then implement the compareTo method which returns positive number if the current object is greater then passed in object and negative number for the reverse of that.  Return zero if the objects are equal.'),
...
2) == is subject to type compatibility check at compile time
   ...
   for (String theString : strings) {
       ...
       ('SPRING', 'MEDIUM', 'What is a Spring IoC container?',
        'The spring IoC creates the objects, wires them together, configures them, and manages their complete lifecycle from creation till destruction. The spring container uses dependency injection (DI) to manage the components that make up an application.'),
       ('JAVA', 'MEDIUM', 'What is the structure of the Java Heap?',
        'The JVM has a heap that is the runtime data area from which memory for all classes instances and arrays are allocated. It is created at the JVM start-up. Heap memory for objects is reclaimed by an automatic memory management system which is known as a garbage collector. Heap memory consists of live and dead objects. Live objects are accessible by the application and will not be subject to garbage collections. Dead objects are those which will never be accessible by the application, but have not been collected by the garbage collector, yet. Such objects occupy the heap memory space until they are eventually collected by the garbage collector.'),
       ('JAVA', 'MEDIUM', 'What is the volatile keyword useful for?',
        'Volatile has semantics for memory, visibility. Basically, the value of a volatile field becomes visible to all readers (other threads in particular) after a right operation completes on it. Without volatile, readers, could see some non-updated value.'),
       ('SPRING', 'MEDIUM', 'What is Controller in Spring framework?',
...
'),
       ('JAVA', 'MEDIUM', 'What is the difference between fail-fast and fail-safe for Iterators?',
        'The iterators fail – safe property works with the clone of the underlying collection and pass, it is not affected by any modification in the collection. All the collection classes in Java.until package or fail fast, while the collection classes in Java.util.concurrent or fail Dash safe fail fast iterators throw a concurrent modification exception while fail Dash safe iterator never throws such an exception.'),
...
Inversion of Control and Dependency Injection are some techniques for getting loose coupling between modules.'),
       ('JAVA', 'HARD', 'What is the difference between Serial and Throughput Garbage collector?',
...
•	CanLoad
•	Resolve'),
       ('ANGULAR', 'EASY', 'What is a Module, and what does it contain?',
        'And angular module is a set of angular basic building blocks like components, directives, services, etc. An app can have more than one module. A module can be created using the @ngModule decorator.'),
...
For example two threads are blocked forever.  Thread 1 is holding Object 1 but needs object 2 to complete processing whereas Thread 2 is holding Object 2 but needs object 1 first. In such conditions, both of them will hold lock forever and will never complete tasks.'),
       ...
       ('JAVA', 'HARD', 'Can two threads execute two methods (static and non-static concurrently)?',
       ...
       ('JAVA', 'HARD', 'What will happen if we don''t override the thread class run() method?',
        'Nothing will happen if we don’t override the run() method. The compiler will not show any error. It will execute the run() method of thread class and we will just not get any output because the run() method is with an empty implementation.'),
       ('JAVA', 'HARD', 'Is it possible for each thread to have its own stack in multithreaded programming?',
        'Yes. In multithreaded programming, each thread maintains its own separate stack area in memory because each thread is independent of each other rather than dependent.'),
       ('JAVA', 'HARD', 'Is it possible to call the run() method directly to start a new thread?',
        'No, it''s not possible to start a thread. You need to call the start() method to create a new thread otherwise run method won''t create a new thread. Calling the run() method it will behave like a normal method and not start a new thread.'),
       ...
       ('JAVA', 'HARD', 'How can we achieve thread safety in Java?',
       ...
       ('JAVA', 'HARD', 'What is more preferred - Synchronized method or Synchronized block?',
        'Synchronized block is the more preferred way because it doesn’t lock the object, synchronized methods lock the object.  If there are multiple synchronization blocks in the class, even though they are not related, it will stop them from executing and put them in wait state to get the lock on object.'),
       ...
       ('JAVA', 'HARD', 'What is a race condition?',
